...
table(pheno$dana, pheno$PARTITION) %>% kable(., caption = "All patients dana") %>% kable_styling(full_width = TRUE)
pheno <- pheno %>% filter(dana1 !="HC")
load("../Phenocleaning/miRData.RData") 

all <- left_join(select(pheno, SampleID, dana, ntprobnp, PARTITION), new, by = "SampleID")
all$dana <- dplyr::recode(all$dana,  PH4 = "target", PH1.4 = "target", PH2 = "target", PH1.1 = "target", PH5 = "target", PH3 = "target", PH1.2 = "target", PH1.3 = "target", PH0_NOPH = "Cont", PH0_CTED = "Cont")
...
kable(table(Combined$dana))
kable(table(Validation$dana)) 
```
...
pROC::ggroc(roc.list.XGB, aes = c("linetype","color")) + scale_color_manual(values = c("deeppink","midnight blue", "cornflowerblue",  "darksalmon")) + scale_linetype_manual(values = c("solid", "solid", "solid", "dashed")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("PH vs DC") + guides(linetype = "none")
...
XGBDIpreds2N %>% write_csv(., "Predictions/PHvsDC2withNTproBNP.csv")

xgbpredsDIN$dana <- Validation$dana
xgbpredsDIN$SampleID <- Validation$SampleID
write_csv(xgbpredsDIN, "Predictions/validationpredsPHvsDCwithNTproBNP.csv")
```
...
```{r}
XGBvarsDI %>% mutate(method = "XGBoost", comparison = "PHvsDC") %>% rename(Importance = XGBIMP) %>% write_csv(., "miRNAs/PHvsDC.csv")
...
XGBDIpreds %>% select(rowIndex, obs.1, grep("^XGB", colnames(XGBDIpreds))) %>% left_join(., select(Combined, rowIndex, SampleID)) %>% write_csv(., "Predictions/PHvsDC.csv")
xgbpredsDI$dana <- Validation$dana
xgbpredsDI$SampleID <- Validation$SampleID
write_csv(xgbpredsDI, "Predictions/validationpredsPHvsDC.csv")
...
XGBDIpreds2 %>% write_csv(., "Predictions/PHvsDC2.csv")
...
xgb_baseTI<- caret::train( 
  ...
  method = "xgbTree",
  verbose = TRUE,
  scale_pos_weight = table(labelsTI)[[2]]/table(labelsTI)[[1]]
  )
...
plotdetail_more <- left_join(plotdetail, select(pheno, SampleID, site, treated), by = "SampleID") %>% mutate(FPFNbinary = ifelse(FPFN > 20, "High missclassified %", "Low misclassified %")) %>% arrange(dana1, FPFNbinary, site, treated) %>% select(-SampleID, -FPFN)  %>% mutate(rowIndex = as.numeric(row.names(.))) %>% melt(., id.vars = "rowIndex") %>% mutate(type = case_when(variable == "dana1" ~ "WHO classification", variable == "FPFNbinary" ~ "Misclassification", variable == "site" ~ "Site", variable == "treated" ~ "Treatment status", TRUE ~ "" )) 
plotdetail_more$type_f <- factor(plotdetail_more$type, levels = c("WHO classification", "", "Misclassification", "Site", "Treatment status"))

ggplot(plotdetail_more, aes(variable, rowIndex)) + geom_tile(aes(fill = value)) + scale_fill_manual(values=c("red", "blue", "black")) + scale_fill_manual(values = c("darksalmon", "grey", "midnight blue", "deeppink", "dark red", "yellow", "grey", "darksalmon", "yellow", "cornflowerblue","cornflowerblue", "grey", "grey",  "dark red")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + facet_grid(~type_f, scales= 'free_x', space = 'free_x')
```
...
XGBDIpredsThresh <- tosplitTI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(tosplitTI$pred$rowIndex)/10)) %>% select(rowIndex, obs, target, timev) %>% mutate(pred.T = ifelse(target > mean(XGBthresholdspecs$threshold), "target", "Cont")) %>%  mutate(XGB = case_when(obs == "Cont" & pred.T == "Cont" ~ "TN", obs == "Cont" & pred.T == "target" ~ "FP", obs == "target" & pred.T == "Cont" ~ "FN", obs == "target" & pred.T == "target" ~ "TP")) %>% reshape(idvar = "rowIndex", timevar = "timev", direction = "wide")
XGBDIpredsThresh <- select(XGBDIpredsThresh, rowIndex, obs.1, grep("^pred.T", colnames(XGBDIpredsThresh)), grep("^XGB", colnames(XGBDIpredsThresh))) 
...
ggplot(plotdetail_moreThresh, aes(variable, rowIndex)) + geom_tile(aes(fill = value)) + scale_fill_manual(values=c("red", "blue", "black")) + scale_fill_manual(values = c("grey", "midnight blue", "deeppink", "dark red", "grey", "cornflowerblue", "midnight blue", "darksalmon", "grey", "grey", "dark red")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + facet_grid(~type_f, scales= 'free_x', space = 'free_x')
```
...
Correlation coef > 0.7 (next highest ~0.4)

```{r}
highcors<- c("hsa.miR.10b.5p", "hsa.miR.30a.5p", "hsa.miR.193b.3p")
...
ggplot(prdata, aes(x = recall, y = precision, group = type)) +
  geom_path(aes(color = type)) +ylim(c(0,1)) + scale_color_manual(values = c("deeppink","midnightblue")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("PH vs DC")


XGBDIpredsThresh <- tosplitTI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(LASSO.1se.DI$pred$rowIndex)/10)) %>% select(rowIndex, obs, target, timev) 

paste("XGB mean threshold:", mean(XGBthresholdspecs$threshold)) #fpr, tnr tpr fnr sensitivity specificity

XGBthresholdsNew<- lapply(1:100, function(d) {
  pROC::coords(XGBlistofrocs[[d]], "best",  "fpr") }) %>% unlist()
XGBthresholdspecsNew <- data.frame(threshold = double(), specificity = double(), sensitivity = double())
for(i in 1:100) {
 XGBthresholdspecsNew[i,] <- unlist(XGBthresholdsNew[i])
}

paste("XGB mean threshold:", median(XGBthresholdspecsNew$threshold))


XGBDIpredsThreshNew <- tosplitTI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(tosplitTI$pred$rowIndex)/10)) %>% select(rowIndex, obs, target, timev) %>% mutate(pred.T = ifelse(target > median(XGBthresholdspecs$threshold), "target", "Cont")) %>%  mutate(XGB = case_when(obs == "Cont" & pred.T == "Cont" ~ "TN", obs == "Cont" & pred.T == "target" ~ "FP", obs == "target" & pred.T == "Cont" ~ "FN", obs == "target" & pred.T == "target" ~ "TP")) %>% reshape(idvar = "rowIndex", timevar = "timev", direction = "wide")
XGBDIpredsThreshNew <- select(XGBDIpredsThreshNew, rowIndex, obs.1, grep("^pred.T", colnames(XGBDIpredsThreshNew)), grep("^XGB", colnames(XGBDIpredsThreshNew))) 

allpredsThresh <- left_join(select(RFDIpredsThresh, rowIndex, observed, grep("^pred.", colnames(RFDIpredsThresh))), select(rpartDIpredsThresh, rowIndex, grep("^pred.", colnames(rpartDIpredsThresh))), by = "rowIndex") %>% left_join(., select(LASSO1seDIpredsThresh, rowIndex, grep("^pred.", colnames(LASSO1seDIpredsThresh))), by = "rowIndex") %>% left_join(., select(XGBDIpredsThreshNew, rowIndex, grep("^pred.", colnames(XGBDIpredsThreshNew))), by = "rowIndex")

recodedpredsThreshM <- left_join(select(RFDIpredsThresh, observed, rowIndex, grep("^RF.", colnames(RFDIpredsThresh))), select(rpartDIpredsThresh, rowIndex, grep("^rpart.", colnames(rpartDIpredsThresh))), by = "rowIndex") %>% left_join(., select(LASSO1seDIpredsThresh, rowIndex, grep("^LASSO", colnames(LASSO1seDIpredsThresh))), by = "rowIndex")%>% left_join(., select(XGBDIpredsThreshNew, rowIndex, grep("^XGB", colnames(XGBDIpredsThreshNew))), by = "rowIndex")

plotdataThreshM <- recodedpredsThreshM %>% arrange(observed)  %>% mutate(rowIndex = as.numeric(row.names(.))) %>% melt(., id.vars = "rowIndex") 

ggplot(plotdataThreshM, aes(variable, rowIndex)) + geom_tile(aes(fill = value)) + scale_fill_manual(values=c("red", "blue", "black")) + scale_fill_manual(values = c("tomato2","midnight blue", "red", "cornflowerblue", "grey", "grey")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
xgbforboxplot <- Combined %>% select(XGBvarsDI$miR, dana) %>% mutate(dana = recode(dana, "target" = "PH", "Cont" = "DC")) %>% pivot_longer(c(-dana), names_to = "miRNA", values_to = "Expression") %>% mutate(miRNA = gsub(pattern = "\\.", replacement = "-", x = miRNA))
