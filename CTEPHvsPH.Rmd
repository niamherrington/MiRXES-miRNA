...
all$dana <- dplyr::recode(all$dana,  PH4 = "target", PH1.4 = "Cont", PH2 = "Cont", PH1.1 = "Cont", PH5 = "Cont", PH3 = "Cont", PH1.2 = "Cont", PH1.3 = "Cont")
...
set.seed(100)
tunegrid <- expand.grid(.predFixed=c(1:5), .minNode = 2)
modellist<- list()
	set.seed(100)
...
b<- as.numeric(a)
hist(b, main = paste("XGBoost below threshold CV mean AUC:", round(mean(b, na.rm = TRUE),2),  "(SD:", round(sd(b, na.rm = TRUE),2), ")"))
...
XGBvarsDI %>% mutate(method = "XGBoost", comparison = "CTEPHvsPH") %>% rename(Importance = XGBIMP) %>% write_csv(., "miRNAs/CTEPHvsPH.csv")
...
XGBDIpreds %>% select(rowIndex, obs.1, grep("^XGB", colnames(XGBDIpreds))) %>% left_join(., select(Combined, rowIndex, SampleID)) %>% write_csv(., "Predictions/CTEPHvsPHXGB.csv")
xgbpredsDI$dana <- Validation$dana
xgbpredsDI$SampleID <- Validation$SampleID
write_csv(xgbpredsDI, "Predictions/validationpredsCTEPHvsPH.csv")
...
XGBDIpreds2N %>% write_csv(., "Predictions/CTEPHvsPHXGB2withNTproBNP.csv")

xgbpredsDIN$dana <- Validation$dana
xgbpredsDIN$SampleID <- Validation$SampleID
write_csv(xgbpredsDIN, "Predictions/validationpredsCTEPHvsPHPAHwithNTproBNP.csv")
```
...
```{r, eval = F}
RFDIpreds0 <- fit.Boruta.DI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(fit.Boruta.DI$pred$rowIndex)/10)) %>% select(rowIndex, obs, "RFpreds" = target, timev) %>% mutate(ML = "RF") 

rpartDIpreds0 <- fit.caret.rpart.DI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(fit.caret.rpart.DI$pred$rowIndex)/10)) %>% select(rowIndex, obs, "rpartpreds" = target, timev) %>% mutate(ML = "rpart") 

LASSO1seDIpreds0 <- LASSO.1se.DI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(LASSO.1se.DI$pred$rowIndex)/10)) %>% select(rowIndex, obs,"LASSOpreds" = target, timev)   %>% mutate(ML = "LASSO") 

XGBDIpreds0 <- tosplitTI$pred %>% arrange(rowIndex) %>% mutate(timev = rep(seq(1,10),length(LASSO.1se.DI$pred$rowIndex)/10)) %>% select(rowIndex, obs, "XGBpreds" = target, timev)  %>% mutate(ML = "XGBoost") 

all0preds <- left_join(select(RFDIpreds0, rowIndex, obs, RFpreds), select(rpartDIpreds0, rowIndex, rpartpreds), by = "rowIndex") %>% left_join(., select(LASSO1seDIpreds0, rowIndex, LASSOpreds)) %>% left_join(., select(XGBDIpreds0, rowIndex, XGBpreds), by = "rowIndex") %>% arrange(obs)  %>% mutate(rowIndex = as.numeric(rowIndex)) %>% melt(., id.vars = "rowIndex") 
all0preds$value <- ifelse(all0preds$value == "target", 1, 0)
all0preds$value <- as.numeric(all0preds$value)
ggplot(all0preds, aes(x = variable, y = rowIndex, fill = value)) + geom_tile()
...
pROC::ggroc(roc.list.XGB, aes = c("linetype","color")) + scale_color_manual(values = c("deeppink","midnight blue", "cornflowerblue",  "darksalmon")) + scale_linetype_manual(values = c("solid", "solid", "solid", "dashed")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("CTEPH vs PH - XGBoost model") + guides(linetype = "none")
...
ggplot(prdata, aes(x = recall, y = precision, group = type)) +
  geom_path(aes(color = type)) +ylim(c(0,1)) + scale_color_manual(values = c("deeppink","midnightblue")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("CTEPH vs PH")
...
```{r, include=FALSE}
save.image(file = "CTEPHvsPH.RData") # 
...
xgbforboxplot <- Combined %>% select(XGBvarsDI$miR, dana) %>% mutate(dana = recode(dana, "target" = "CTEPH", "Cont" = "Other PH")) %>% pivot_longer(c(-dana), names_to = "miRNA", values_to = "Expression") %>% mutate(miRNA = gsub(pattern = "\\.", replacement = "-", x = miRNA))
