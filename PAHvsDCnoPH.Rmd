...
PAH vs DC (no PH), MiRXES data set
...
```{r, warning=FALSE, message=FALSE}
#Data processing
pheno <- read_csv("../Phenocleaning/phenotoclean.csv")  %>% filter(dana %in% c("PH1.1", "PH1.2", "PH1.3", "PH1.4", "PH0_NOPH", "PH0_CTED"))
load("../Phenocleaning/miRData.RData") 

all <- left_join(select(pheno, SampleID, dana, ntprobnp, PARTITION), new, by = "SampleID")
all$dana <- dplyr::recode(all$dana, PH1.4 = "target", PH1.1 = "target",  PH1.2 = "target", PH1.3 = "target", PH0_NOPH = "Cont", PH0_CTED = "Cont")
...
```{r}
export$comparison <- "PAH vsDC noPH"
...
plotdata <- melt(recodedpreds, id.vars = "rowIndex")
...
plot2 <- as.matrix(select(recodedpreds, -rowIndex))
ha = rowAnnotation(name = as.character(recodedpreds$observed))
Heatmap(plot2[-1], left_annotation = ha)
```
...
paste("rpart CV AUC above NT-proBNP threshold", pROC::auc(pROC::roc(predictor = CV_RFA$target, response = CV_RFA$obs)))
paste("rpart CV AUC below NT-proBNP threshold", pROC::auc(pROC::roc(predictor = CV_RFB$target, response = CV_RFB$obs)))
...
pROC::ggroc(roc.list.XGB, aes = c("linetype","color")) + scale_color_manual(values = c("deeppink","midnight blue", "cornflowerblue",  "darksalmon")) + scale_linetype_manual(values = c("solid", "solid", "solid", "dashed")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("PAH vs DC") + guides(linetype = "none")
...
paste("rpart CV AUC above NT-proBNP /age threshold", pROC::auc(pROC::roc(predictor = CV_RFa$target, response = CV_RFa$obs)))
paste("rpart CV AUC below NT-proBNP /age threshold", pROC::auc(pROC::roc(predictor = CV_RFb$target, response = CV_RFb$obs)))
...
```{r}
XGBvarsDI %>% mutate(method = "XGBoost", comparison = "PAHvsDCnoPH") %>% rename(Importance = XGBIMP) %>% write_csv(., "miRNAs/PAHvsDCnoPH.csv")
...
XGBDIpreds %>% select(rowIndex, obs.1, grep("^XGB", colnames(XGBDIpreds))) %>% left_join(., select(Combined, rowIndex, SampleID)) %>% write_csv(., "Predictions/PAHvsDCnoPHXGB.csv")
xgbpredsDI$dana <- Validation$dana
xgbpredsDI$SampleID <- Validation$SampleID
write_csv(xgbpredsDI, "Predictions/validationpredsPAHvsDCnoPH.csv")
...
XGBDIpreds2N %>% write_csv(., "Predictions/PH2PH5vsDCPAHXGB2withNTproBNP.csv")

xgbpredsDIN$dana <- Validation$dana
xgbpredsDIN$SampleID <- Validation$SampleID
write_csv(xgbpredsDIN, "Predictions/validationpredsPH2PH5vsDCPAHwithNTproBNP.csv")
```
...
## Logistic regression on BWH data

### Rest

```{r, message=FALSE, warning=FALSE}
peak.short <- peak %>% filter(dana %in% c("NHC", "PH1")) 
post.short <- post %>% filter(dana %in% c("NHC", "PH1")) 
rest.short <- rest  %>% filter(dana %in% c("NHC", "PH1")) 

#NA value in hsa.miR.193b.3p for patient 1314 (rest only). Patient removed for rest
restXGBLR <- rest.short %>% select(patient, dana, all_of(XGBvarsDI$miR)) %>% filter(patient != 1314)
peakXGBLR <- peak.short %>% select(patient, dana, all_of(XGBvarsDI$miR))
postXGBLR <- post.short %>% select(patient, dana, all_of(XGBvarsDI$miR))
...
peak.early <- peak %>% filter(dana %in% c("NHC", "ePH", "blPH"))   %>% select(patient, dana, all_of(XGBvarsDI$miR)) %>% filter(patient != 1314)
post.early <- post %>% filter(dana %in% c("NHC", "ePH", "blPH"))  %>% select(patient, dana, all_of(XGBvarsDI$miR))
rest.early <- rest  %>% filter(dana %in% c("NHC", "ePH", "blPH"))   %>% select(patient, dana, all_of(XGBvarsDI$miR)) %>% filter(patient != 1314)
peak.early$dana <- dplyr::recode(peak.early$dana, ePH = "earlyPH", blPH = "earlyPH",  NHC = "NHC")
post.early$dana <- dplyr::recode(post.early$dana, ePH = "earlyPH", blPH = "earlyPH",  NHC = "NHC")
rest.early$dana <- dplyr::recode(rest.early$dana, ePH = "earlyPH", blPH = "earlyPH",  NHC = "NHC")
...
peak.all <- peak %>% filter(dana %in% c("NHC", "ePH", "blPH", "PH1"))   %>% select(patient, dana, all_of(XGBvarsDI$miR)) %>% filter(patient != 1314)
post.all <- post %>% filter(dana %in% c("NHC", "ePH", "blPH", "PH1"))  %>% select(patient, dana, all_of(XGBvarsDI$miR))
rest.all <- rest  %>% filter(dana %in% c("NHC", "ePH", "blPH", "PH1"))   %>% select(patient, dana, all_of(XGBvarsDI$miR)) %>% filter(patient != 1314)
peak.all$dana <- dplyr::recode(peak.all$dana, ePH = "PAH", blPH = "PAH", PH1 = "PAH", NHC = "NHC")
post.all$dana <- dplyr::recode(post.all$dana, ePH = "PAH", blPH = "PAH", PH1 = "PAH", NHC = "NHC")
rest.all$dana <- dplyr::recode(rest.all$dana, ePH = "PAH", blPH = "PAH", PH1 = "PAH", NHC = "NHC")
...
# Early PH vs NHC on mean centred data 
...
# Early PH and PAH vs NHC on mean centred data 
...
LRmcAcoefs$miRNA <- gsub("\\.", "-", LRmcAcoefs$miRNA)
...
Figure <- rbind(LRmc, restLRmc,restLRmcE) %>% mutate(comparison = factor(comparison, levels = c("D+I", "Rest, PAH", "Rest, Early PAH" )))

Figure$miRNA <- gsub("\\.", "-", Figure$miRNA)
Figure <- Figure %>% arrange(comp, abs(Estimate)) %>% mutate(miRNA = factor(miRNA, levels = head(miRNA, 10)))
...
Figure %>% ggplot(aes(x=miRNA, y = Estimate)) + geom_col(aes(fill=comp), show.legend = FALSE) + theme(axis.text.x=element_text(angle=90, vjust=0.5, size = 14), panel.background = element_blank(), panel.grid = element_line(colour="grey"), strip.text = element_text(size = 14)) + labs(x="", y="Coefficient", size=14) + scale_fill_manual(name = "", values = c("deeppink", "pink", "midnight blue", "cornflowerblue")) +  facet_wrap(~comp, scales = "free_x", nrow = 1) + scale_x_reordered() 

Figure %>% filter(comp != "Rest, Early PAH") %>% ggplot(aes(x=miRNA, y = Estimate)) + geom_col(aes(fill=comp), show.legend = FALSE) + theme(axis.text.x=element_text(angle=90, vjust=0.5, size = 14), panel.background = element_blank(), panel.grid = element_line(colour="grey"), strip.text = element_text(size = 14)) + labs(x="", y="Coefficient", size=14) + scale_fill_manual(name = "", values = c("deeppink",  "midnight blue")) +  facet_wrap(~comp, scales = "free_x", nrow = 1) + scale_x_reordered() 
...
XGBLRmcErest$Comparison <- "Rest, Early PH1"
...
XGBLRmcrest$Comparison <- "Rest, PH1"
...
patientscores$dana <- dplyr::recode(patientscores$dana,  target = "PH1", Cont = "DC", PH2 = "Cont", NHC = "DC", earlyPH = "early PH1")
...
patientscores %>% filter(Comparison != "Rest, Early PAH") %>% ggplot(aes(x = dana, y = target)) + geom_boxplot(aes(fill = Comparison), show.legend = FALSE) + geom_jitter() + theme(axis.text.x=element_text(angle=90, vjust=0.5, size = 14), panel.background = element_blank(), panel.grid = element_line(colour="grey"), strip.text = element_text(size = 14)) + scale_fill_manual(name = "", values = c("deeppink", "pink", "midnight blue")) + labs(x = "", y = "LR Patient Score") + facet_wrap(~Comparison, scales = "free_x", nrow = 1)
...
ggplot(prdata, aes(x = recall, y = precision, group = type)) +
  geom_path(aes(color = type)) +ylim(c(0,1)) + scale_color_manual(values = c("deeppink","midnightblue")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("PAH vs DC")

```

# Mean centred values in all 3 datasets

```{r}
TrainXGBLRmc$dataf <- "PAH D+I (UK)"
ValXGBLRmc$dataf <- "PAH Validation (UK)"
restXGBLRmc$dataf <- "PAH (US)"
restXGBLRmc <- restXGBLRmc %>% rename(SampleID = patient)
miRmc <- rbind(TrainXGBLRmc, ValXGBLRmc, restXGBLRmc)

miRmc$dana <- dplyr::recode(miRmc$dana, Cont = "DC", target = "PAH", NHC = "DC", PH1 = "PAH")
colnames(miRmc) <- gsub("\\.",'-', colnames(miRmc))

miRmclong <- miRmc %>% select(-SampleID) %>% pivot_longer(., cols = c("hsa-miR-151a-5p", "hsa-miR-210-3p",   "hsa-miR-193b-3p", "hsa-miR-30a-5p", "hsa-miR-126-3p",  "hsa-miR-26a-2-3p", "hsa-miR-200c-3p", "hsa-miR-10b-5p",  "hsa-miR-1226-3p" ))

miRmclong$dataf <- factor(miRmclong$dataf, levels = c("PAH D+I (UK)", "PAH Validation (UK)", "PAH (US)"))

ggplot(miRmclong, aes(x = dataf, y = value, fill = factor(dana))) + geom_boxplot() + facet_wrap(~name) + labs(x = "", y = "", fill = "DANA") + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_fill_manual(values = c("deeppink", "midnightblue"))

mediansDI <- TrainXGBLRmc %>% select(-SampleID, -dataf) %>% group_by(dana) %>% summarise(across(where(is.numeric), ~median(.x))) %>% t() %>% as.data.frame() %>% mutate(Group = "PAH UK D+I")
mediansV <- ValXGBLRmc %>% select(-SampleID, -dataf) %>% group_by(dana) %>% summarise(across(where(is.numeric), ~median(.x)))  %>% t() %>% as.data.frame() %>% mutate(Group = "PAH UK V")
mediansUS <- restXGBLRmc %>% select(-SampleID, -dataf) %>% group_by(dana) %>% summarise(across(where(is.numeric), ~median(.x)))  %>% t() %>% as.data.frame() %>% mutate(Group = "PAH US")

meds<- bind_rows(mediansDI, mediansV, mediansUS) %>% rename("Cont" = V1, "target" = V2) %>% filter(Cont != "Cont") %>% filter(target != "PH1") %>% mutate(Cont = as.numeric(Cont), target = as.numeric(target)) %>% mutate(diff = Cont-target)
```
...
Pick out miRNAs from interesting pathways and heatmap to ullustrate where high and low

```{r, include=FALSE, cache = FALSE}
save.image(file = "PAHvsDCnoPH.RData") # 
...
#miRlevels(Combined$hsa.miR.151a.5p, "hsa-miR-151a-5p")

xgbforboxplot <- Combined %>% select(XGBvarsDI$miR, dana) %>% mutate(dana = recode(dana, "target" = "PAH", "Cont" = "DC")) %>% pivot_longer(c(-dana), names_to = "miRNA", values_to = "Expression") %>% mutate(miRNA = gsub(pattern = "\\.", replacement = "-", x = miRNA))
...
xgbforboxplot %>% filter(miRNA %in% c("hsa-miR-10b-5p", "hsa-miR-1226-3p", "hsa-miR-126-3p")) %>% ggplot(aes(x=dana, y = Expression, fill = dana)) + geom_boxplot() + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"), text = element_text(size = 14)) + labs(x = "") + facet_wrap(~miRNA, scales = "free") + scale_fill_manual(values = c("deeppink", "midnight blue"))

xgbforboxplot %>% filter(miRNA %in% c("hsa-miR-151a-5p", "hsa-miR-193b-3p", "hsa-miR-200c-3p")) %>% ggplot(aes(x=dana, y = Expression, fill = dana)) + geom_boxplot() + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"), text = element_text(size = 14)) + labs(x = "") + facet_wrap(~miRNA, scales = "free") + scale_fill_manual(values = c("deeppink", "midnight blue"))

xgbforboxplot %>% filter(miRNA %in% c("hsa-miR-210-3p", "hsa-miR-26a-2-3p", "hsa-miR-30a-5p")) %>% ggplot(aes(x=dana, y = Expression, fill = dana)) + geom_boxplot() + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"), text = element_text(size = 14)) + labs(x = "") + facet_wrap(~miRNA, scales = "free") + scale_fill_manual(values = c("deeppink", "midnight blue"))
