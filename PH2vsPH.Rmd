...
all$dana <- dplyr::recode(all$dana,  PH4 = "Cont", PH1.4 = "target", PH2 = "target", PH1.1 = "Cont", PH5 = "Cont", PH3 = "Cont", PH1.2 = "Cont", PH1.3 = "Cont")
...
xgb_base<- caret::train( 
  ...
  method = "xgbTree",
  verbose = TRUE,
  scale_pos_weight = table(labels)[[2]]/table(labels)[[1]]
  )
...
pROC::ggroc(roc.list.XGB, aes = c("linetype","color")) + scale_color_manual(values = c("deeppink","midnight blue", "cornflowerblue",  "darksalmon")) + scale_linetype_manual(values = c("solid", "solid", "solid", "dashed")) + theme(text = element_text(size = 14), panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"))  +  guides(fill=guide_legend(title=""))  + labs(color = "") + ggtitle("PH2 vs PH - XGBoost model") + guides(linetype = "none")
...
```{r}
RFvarsDI %>% mutate(method = "RF", comparison = "PH2vsPH") %>% rename(Importance = RFIMP) %>% write_csv(., "miRNAs/PH2vsPH.csv")
...
XGBDIpreds %>% select(rowIndex, obs.1, grep("^XGB", colnames(XGBDIpreds))) %>% left_join(., select(Combined, rowIndex, SampleID)) %>% write_csv(., "Predictions/PH2vsPHXGB.csv")
xgbpredsDI$dana <- Validation$dana
xgbpredsDI$SampleID <- Validation$SampleID
write_csv(xgbpredsDI, "Predictions/validationpredsPH2vsPH.csv")
...
XGBDIpreds2 %>% write_csv(., "Predictions/PH2vsPHXGB2.csv")
...
XGBDIpreds2N %>% write_csv(., "Predictions/PH2vsPHXGB2withNTproBNP.csv")

xgbpredsDIN$dana <- Validation$dana
xgbpredsDIN$SampleID <- Validation$SampleID
write_csv(xgbpredsDIN, "Predictions/validationpredsPH2vsPHwithNTproBNP.csv")
```
...
```{r}
rfforboxplot <- Combined %>% select(RFvarsDI$miR, dana) %>% mutate(dana = recode(dana, "target" = "PH-LHD", "Cont" = "Other PH")) %>% pivot_longer(c(-dana), names_to = "miRNA", values_to = "Expression") %>% mutate(miRNA = gsub(pattern = "\\.", replacement = "-", x = miRNA))

ggplot(rfforboxplot, aes(x=dana, y = Expression, fill = dana)) + geom_boxplot() + theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey"), axis.line = element_line(colour = "grey"), text = element_text(size = 14)) + labs(x = "") + facet_wrap(~miRNA, scales = "free") + scale_fill_manual(values = c("deeppink", "midnight blue"))
```
